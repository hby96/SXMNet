...
def make_dataset_from_xml(dir):
    images = []
    img_list = os.listdir(os.path.join(dir))
    for img in img_list:
...
class MyDatasetFolderWoLabel(data.Dataset):
    ...
    def __init__(self, root, loader, extensions, transform=None, target_transform=None):
        ...
        samples = make_dataset_wo_label(root)
        print(len(samples))
...
class MyFolderWoLabel(MyDatasetFolderWoLabel):

    def __init__(self, root, transform=None, target_transform=None,
                 loader=default_loader):
        super(MyFolderWoLabel, self).__init__(root, loader, IMG_EXTENSIONS,
                                       transform=transform,
                                       target_transform=target_transform)
        self.imgs = self.samples
