...
# object_categories = ['Gun', 'Knife', 'Liquid', 'Scissors', 'Z-Safe']
...
# mask_root_path = '/home/hubenyi/Multi-Label-Series/My-CHR/data_set/attention_dataset/train_test_split/train/Ellipse_Heatmaps/'
# mask_root_path = '../data_set/attention_dataset/train_test_split/train/Gau_Point_Heatmaps/'
mask_root_path = '/home/hby/Documents/Datasets/Top_View_Xray/train_test_split/train/Ellipse_Heatmaps/'
    ...
    for img in img_list:
        ...
        if 'safe' in img:
            ...
            for i in range(len(object_categories)):
                mask_path = os.path.join(mask_root_path, object_categories[i], img)
                if os.path.exists(mask_path):
                    mask_path_list.append(mask_path)
            total_mask_path = os.path.join(mask_root_path, 'ZZ-Total', img)
            if os.path.exists(total_mask_path):
                    mask_path_list.append(total_mask_path)

            # item = (img_path, label, mask_path_list)
            # images.append(item)
        else:
...
class MyDatasetFolder(data.Dataset):
    ...
    def __init__(self, root, loader, extensions, transform=None, ht_transform=None, target_transform=None):
        ...
        scissors_xray_num = 0
        safe_xray_num = 0
        for (img, label, bbox_list) in samples:
            if label[0] >= 0.89:
                battery_xray_num += 1
            if label[1] >= 0.89:
                bottle_xray_num += 1
    ...
    def __getitem__(self, index):
        ...
        path, target, mask_path_list = self.samples[index]
        # print('path:', path)
        ...
        if self.ht_transform is not None:
            if len(mask_path_list) > 0:
                mask_sample_list = [self.loader(x) for x in mask_path_list]
                for i in range(len(mask_sample_list)):
                    random.seed(seed)
                    mask_sample_list[i] = self.ht_transform(mask_sample_list[i])
                return sample, target, mask_sample_list[-1]
            else:
                print(path)
                assert False
        else:
...
class MyFolder(MyDatasetFolder):
    ...
    def __init__(self, root, transform=None, ht_transform=None, target_transform=None,
                 loader=default_loader):
        super(MyFolder, self).__init__(root, loader, IMG_EXTENSIONS,
                                       transform=transform,
                                       ht_transform=ht_transform,
